...
function renderTabelaCarreiras() {
  ...
  if (!window._carreirasDropdownInit) {
    document.addEventListener("click", () => {
      document.querySelectorAll(".dropdown.open").forEach((d) => closeDrop(d));
    });
  ...
  tbody.querySelectorAll("[data-export]").forEach((btn) => {
    btn.onclick = (e) => {
      ...
      if (!carreira) return;
      const blob = new Blob(
        [JSON.stringify({ nome: carreira.nome, save: carreira.save }, null, 2)],
  ...
  reader.onload = function (evt) {
    try {
      ...
      if (carreiras.some((c) => c.nome === data.nome)) {
        if (!confirm("Já existe uma carreira com esse nome. Sobrescrever?"))
          return;
...
// Importar carreira (abre input)
document.getElementById("btn-importar-carreira").onclick = () => {
  document.getElementById("input-importar-save").click();
};
